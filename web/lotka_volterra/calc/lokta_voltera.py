...
def draw_plot(a, b, c, d, init_x, init_y, calc_id):
    ...
    X0 = array([init_x, init_y])                     # initials conditions: 10 rabbits and 5 foxes
    ...
    f1 = p.figure()
    p.plot(t, rabbits, 'r-', label=_('Показатель рентабельности ТУ'))
    p.plot(t, foxes  , 'b-', label=_('Жесткость регулирования'))
    p.grid()
    p.legend(loc='best')
    p.xlabel(_('время'))
    p.ylabel(_('рентабельность услуг'))
    p.title(_('Зависимость жесткости регулирования и рентабельности ТУ'))
    f1.savefig(os.path.join(settings.BASE_DIR, 'lotka_volterra', 'static', 'graphs') + '/rabbits_and_foxes_1_{}.png'.format(calc_id))
    f1.savefig(os.path.join(settings.BASE_DIR, 'staticfiles', 'graphs') + '/rabbits_and_foxes_1_{}.png'.format(calc_id))
    #
    #
    ...
    # the growth speed
    p.title(_('Фазовый портрет и линии уровня'))
    Q = p.quiver(X1, Y1, DX1, DY1, M, pivot='mid', cmap=p.cm.jet)
    p.xlabel(_('Рентабельность транспортных услуг'))
    p.ylabel(_('Жесткость регулирования'))
    p.legend()
    p.grid()
    p.xlim(0, xmax)
    p.ylim(0, ymax)
    f2.savefig(os.path.join(settings.BASE_DIR, 'lotka_volterra', 'static', 'graphs') + '/rabbits_and_foxes_2_{}.png'.format(calc_id))
    f2.savefig(os.path.join(settings.BASE_DIR, 'staticfiles', 'graphs') + '/rabbits_and_foxes_2_{}.png'.format(calc_id))
    #
    #
    ...
    p.xlim(1, xmax)
    p.title('IF contours')
    f3.savefig(os.path.join(settings.BASE_DIR, 'lotka_volterra', 'static', 'graphs') + '/rabbits_and_foxes_3_{}.png'.format(calc_id))
    f3.savefig(os.path.join(settings.BASE_DIR, 'staticfiles', 'graphs') + '/rabbits_and_foxes_3_{}.png'.format(calc_id))
    # p.show()
    #
    #
